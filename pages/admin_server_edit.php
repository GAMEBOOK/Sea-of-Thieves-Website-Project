...
if(!empty($_POST)) {
	...
	if(empty($_SESSION['error'])) {
		...
		$stmt = $database->prepare("UPDATE `servers` SET `name` = ?, `address` = ?, `category_id` = ?, `country_code` = ?, `youtube_id` = ?, `description` = ? WHERE `server_id` = {$server->data->server_id}");
		$stmt->bind_param('ssssss', $name, $new_address, $category_id, $country_code, $youtube_id, $description);
		$stmt->execute();
