...
if(!empty($_POST)) {
	...
	$stmt = $database->prepare("UPDATE `settings` SET `title` = ?, `meta_description` = ?, `analytics_code` = ?, `banned_words` = ?, `email_confirmation` = ?, `servers_pagination` = ?, `avatar_max_size` = ?, `cover_max_size` = ?, `contact_email` = ?, `new_servers_visibility` = ?, `top_ads` = ?, `bottom_ads` = ?, `side_ads` = ?, `out_ads` = ?, `public_key` = ?, `private_key` = ?, `paypal_email` = ?, `payment_currency` = ?, `maximum_slots` = ?, `per_day_cost` = ?, `minimum_days` = ?, `maximum_days` = ?, `facebook` = ?, `twitter` = ?, `googleplus` = ?  WHERE `id` = 1");
	$stmt->bind_param('sssssssssssssssssssssssss', $_POST['title'], $_POST['meta_description'], $_POST['analytics_code'], $_POST['banned_words'], $_POST['email_confirmation'], $_POST['servers_pagination'], $_POST['avatar_max_size'], $_POST['cover_max_size'], $_POST['contact_email'], $_POST['new_servers_visibility'], $_POST['top_ads'], $_POST['bottom_ads'], $_POST['side_ads'], $_POST['out_ads'], $_POST['public_key'], $_POST['private_key'], $_POST['paypal_email'], $_POST['payment_currency'], $_POST['maximum_slots'], $_POST['per_day_cost'], $_POST['minimum_days'], $_POST['maximum_days'], $_POST['facebook'], $_POST['twitter'], $_POST['googleplus']);
	$stmt->execute(); 
	$stmt->close();
...
<ul class="nav nav-pills">
	...
	<li><a href="#recaptcha" data-toggle="tab"><?php echo $language['forms']['recaptcha_settings']; ?></a></li>
...
<form action="" method="post" role="form">
	<div class="tab-content">
		...
		<div class="tab-pane fade" id="ads">
			...
			<div class="form-group">
				<label><?php echo $language['forms']['settings_out_ads']; ?></label>
				<textarea class="form-control" name="out_ads"><?php echo $settings->out_ads; ?></textarea>
			</div>
		</div>
